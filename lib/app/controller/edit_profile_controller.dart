...
class EditProfileController extends GetxController implements GetxService {
  ...
  Future<void> getProfile() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (body != null && body != '') {
        ...
        cover = body['cover'] ?? '';
        current = body['current'] == 'active' ? true : false;
        update();
      }
    } else {
  ...
  Future<void> updateProfileData() async {
    ...
    var param = {'first_name': firstName.text, 'last_name': lastName.text, 'cover': cover, 'mobile': mobileNumber.text, 'id': parser.getUID(), 'current': current == true ? 'active' : 'busy'};
    Response response = await parser.updateProfile(param);
    isLogin.value = !isLogin.value;
    if (response.statusCode == 200) {
