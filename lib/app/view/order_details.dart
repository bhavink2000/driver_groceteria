                    ...
                    child: Row(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Text('Order Cancelled by user'.tr, style: const TextStyle(color: ThemeProvider.redColor, fontSize: 14)),
                        )
                      ],
                    ),
                  ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.symmetric(vertical: 10),
                          child: Text('Order Delivered'.tr, style: const TextStyle(color: ThemeProvider.appColor, fontSize: 14)),
                        )
                      ],
                    ),
                  ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.symmetric(vertical: 10),
                          child: Text('Order Rejected'.tr, style: const TextStyle(color: ThemeProvider.redColor, fontSize: 14)),
                        )
                      ],
                    ),
                  ),
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                                          ...
                                          children: value.details.orders![orderIndex].variations == null
                                              ? [
                                                  ...
                                                  if (value.details.orders![orderIndex].haveGram == 1)
                                                    Text('${value.details.orders![orderIndex].gram} ${'grams'.tr}',
                                                    ...
                                                    Text('${value.details.orders![orderIndex].kg} ${'kg'.tr}', style: const TextStyle(fontSize: 10, fontFamily: 'regular'), textAlign: TextAlign.start)
                                                  else if (value.details.orders![orderIndex].haveLiter == 1)
                                                    Text('${value.details.orders![orderIndex].liter} ${'ltr'.tr}',
                                                        style: const TextStyle(fontSize: 10, fontFamily: 'regular'), textAlign: TextAlign.start)
                                                  else if (value.details.orders![orderIndex].haveMl == 1)
                                                    Text('${value.details.orders![orderIndex].ml} ${'ml'.tr}', style: const TextStyle(fontSize: 10, fontFamily: 'regular'), textAlign: TextAlign.start)
                                                  else if (value.details.orders![orderIndex].havePcs == 1)
                                                    Text('${value.details.orders![orderIndex].pcs} ${'pcs'.tr}',
                                                        style: const TextStyle(fontSize: 10, fontFamily: 'regular'), textAlign: TextAlign.start),
                                                  const Text(' - '),
                        ...
                        Container(
                          ...
                          width: double.infinity,
                          decoration: bottomBorder(),
                          child: Text('Basic Detail'.tr, style: boldText()),
                        ),
                        Container(
                        ...
                        Container(
                          ...
                          width: double.infinity,
                          decoration: bottomBorder(),
                          child: Text('Order Tracking'.tr, style: boldText()),
                        ),
                        Container(
                        ...
                        value.userDetails.firstName != null
                            ? InkWell(
                                onTap: () {
                                  value.openActionModalStore('${value.userDetails.firstName} ${value.userDetails.lastName}', value.userDetails.email.toString(), value.userDetails.mobile.toString(),
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: <Widget>[
                                              ...
                                              children: <Widget>[
                                                ...
                                                Row(
                                                  children: <Widget>[
                                                    ...
                                                    Text(value.userDetails.email.toString()),
                                                  ],
                                                ),
                                                const SizedBox(height: 5),
                                                Row(
                                                  children: <Widget>[
                                                    ...
                                                    const SizedBox(width: 5),
                                                    Text(value.userDetails.mobile.toString()),
                                                  ],
                                                ),
                                              ],
                                            ),
                                      ...
                                      value.orderStatus == 'ongoing' || value.orderStatus == 'accepted'
                                          ? IconButton(onPressed: () => value.openTrackingFromUser(), icon: const Icon(Icons.near_me_sharp, color: ThemeProvider.ratingColor))
                                          : const SizedBox()
                                    ],
                                  ),
                                ),
                            ...
                            : const SizedBox(),
                        value.userDetails.firstName != null ? const SizedBox(height: 16) : const SizedBox(),
                        value.storesDetails.name != null
                            ? Container(padding: const EdgeInsets.symmetric(vertical: 10), width: double.infinity, decoration: bottomBorder(), child: Text('Store Information'.tr, style: boldText()))
                            : const SizedBox(),
                        value.storesDetails.name != null
                            ? InkWell(
                                onTap: () {
                                  value.openActionModalStore(
                                      '${value.storesDetails.name}', value.storesDetails.email.toString(), value.storesDetails.mobile.toString(), value.storesDetails.id.toString());
                                },
                                child: Container(
                                  padding: const EdgeInsets.symmetric(vertical: 16),
                                  child: Row(
                                    ...
                                    children: <Widget>[
                                        ...
                                        children: [
                                          ClipRRect(
                                            child: SizedBox.fromSize(
                                              size: const Size.fromRadius(30),
                                              child: FadeInImage(
                                                image: NetworkImage('${Environments.apiBaseURL}storage/images/${value.storesDetails.cover}'),
                                                placeholder: const AssetImage("assets/images/placeholder.jpeg"),
                                                imageErrorBuilder: (context, error, stackTrace) {
                                                  return Image.asset('assets/images/notfound.png', fit: BoxFit.fitWidth);
                                                },
                                          ...
                                          Container(
                                            ...
                                            child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: <Widget>[
                                                Text('${value.storesDetails.name}', style: boldText()),
                                                const SizedBox(height: 5),
                                                Row(
                                                  children: <Widget>[
                                                    const Icon(Icons.mail, size: 17),
                                                    const SizedBox(width: 5),
                                                    Text(value.storesDetails.address!.length > 25 ? '${value.storesDetails.address!.substring(0, 25)}...' : value.storesDetails.address.toString()),
                                                  ],
                                                ),
                                                const SizedBox(height: 5),
                                                Row(
                                                  children: <Widget>[
                                                    const Icon(Icons.call, size: 17),
                                                    const SizedBox(width: 5),
                                                    Text(value.storesDetails.mobile.toString()),
                                                  ],
                                                ),
                                              ],
                                            ),
                                      ...
                                      value.orderStatus == 'ongoing' || value.orderStatus == 'accepted'
                                          ? IconButton(onPressed: () => value.openTrackingFromStore(), icon: const Icon(Icons.near_me_sharp, color: ThemeProvider.ratingColor))
                                          : const SizedBox()
                                    ],
                                  ),
                                ),
